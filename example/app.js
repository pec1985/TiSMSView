...
function random2()
{
	...
	phrases[9] = "and fell over dead.";
		...
		var personOne = names[personOneNumber]; // first name
		
		var personTwoNumber = personOneNumber; // sets the first name = to the second name for testing purposes
		while (personTwoNumber == personOneNumber) {
			personTwoNumber = (Math.floor(Math.random () * names.length));
			var personTwo = names[personTwoNumber]; // second name
		}
		
		var verbNumber = (Math.floor(Math.random () * verbs.length));
		var verbPairing = verbs[verbNumber]; // gives us the verb
		
		var phrasePairingNumber = (Math.floor(Math.random () * phrases.length));
		var phrasePairing = phrases[phrasePairingNumber]; // gives us the end phrase
...
tf.addEventListener('buttonClicked', function(e){
					...
					setTimeout(function(){
							   switch(x)
							   {
									case 0:tf.recieveMessage(random2());x=1;break;
									case 1:tf.recieveMessage(random1());x=0;break;
							   }
							},1000);
});
